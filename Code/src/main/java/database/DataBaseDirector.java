...
private ObservableList<Section> sectionList = FXCollections.observableArrayList();
	...
	public void setSectionList(ObservableList<Section> sectionList) {
...
public void init(){
		...
		try {
			sections = sectionDao.queryForAll(Section.class);
		...
		sections.forEach(c -> {
			            ...
			            section.setID(c.getID());
			            section.setName(c.getName());
			            section.setGrubosc_srodnika(c.getGrubosc_srodnika());
			            section.setGrubosc_stopki(c.getGrubosc_stopki());
			            section.setMoment_bezwladnosci_y(c.getMoment_bezwladnosci_y());
			            section.setMoment_bezwladnosci_z(c.getMoment_bezwladnosci_z());
			            section.setPole_powierzchni(c.getPole_powierzchni());
			            section.setPromien_bezwladnosci_y(c.getPromien_bezwladnosci_y());
			            section.setPromien_bezwladnosci_z(c.getPromien_bezwladnosci_z());			            
			            section.setPromien_zaokraglenia(c.getPromien_zaokraglenia());
			            section.setSzerokosc_stopki(c.getSzerokosc_stopki());
			            section.setWskaznik_sprezysty_przekroju(c.getWskaznik_sprezysty_przekroju());
			            section.setWysokosc_przekroju(c.getWysokosc_przekroju());
	...
	public data.Section insert(String name){
		...
		for(int i=0;i<39;i++)
			if (getSectionList().get(i).getName().equals(name)) {
